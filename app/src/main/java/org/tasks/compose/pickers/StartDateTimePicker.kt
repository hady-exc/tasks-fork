...
object StartDateTimePicker {
    // This is a copy of same declarations in the StartDatePicker
    const val NO_DAY = 0L
    const val NO_TIME = 0
    ...
    fun StartDateTimePicker(
        ...
        fun returnDate(day: Long = selectedDay, time: Int = selectedTime) {
            if (day != selectedDay || time != selectedTime) { updateValues(day, time) }
            if (autoclose) accept()
        }
        ...
        DatePickerBottomSheet(
            ...
            timeShortcuts = {
                TimeShortcuts(
                    ...
                    today = today,
                    preferences,
                    returnDate = { day, time -> returnDate(day, time) }
                )
            }
        )
        ...
        LaunchedEffect(state.selectedDateMillis) {
            ...
            state.selectedDateMillis?.let { returnDate(day = it - DateTime(it).offset) }
        }
    }

    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    private fun TimeShortcuts(
        selectedDay: Long,
        selectedTime: Int,
        today: DateTime,
        preferences: Preferences,
        returnDate: (Long, Int) -> Unit
    ) {
        fun returnSelectedTime(millisOfDay: Int) {
            ...
            returnDate(day, millisOfDay)
        }

        var showTimePicker by rememberSaveable { mutableStateOf(false) }
        if (showTimePicker) {
            val time = if (selectedTime < 0
